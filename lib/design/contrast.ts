...
import { converter, formatHex, parse, type Rgb } from 'culori'
...
// ─── color string -> bytes (0-255 triple) ────────────────────────────────────
// OKLCH e formato canonical (CLAUDE.md design-tokens), mas presets upstream
// tweakcn vem em HEX/HSL. Delegamos parsing pra culori, que cobre oklch/hex/
// hsl/rgb/hwb/etc. Format-agnostic mantem APCA gate funcionando enquanto a
// migracao pra OKLCH-only nos presets acontece (DEFERRED).
const toRgb = converter('rgb')

function colorToBytes(colorStr: string): readonly [number, number, number] {
  const parsed = parse(colorStr)
  if (!parsed) {
    throw new Error(`Invalid color string: ${colorStr}`)
  }
  const sample = toRgb(parsed) as Rgb
  return [
...
export function apca(fgOklch: string, bgOklch: string): number {
  const fgBytes = colorToBytes(fgOklch) as unknown as [number, number, number]
  const bgBytes = colorToBytes(bgOklch) as unknown as [number, number, number]
