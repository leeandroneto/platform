// RESEARCH: tweakcn (Apache-2.0) — copied from utils/fonts/index.ts
// See NOTICE.md.
// ADAPT: import path for FontCategory/FontInfo → @/lib/design/contract/fonts

import type { FontCategory, FontInfo } from '@/lib/design/contract/fonts'
...
export function extractFontFamily(fontFamilyValue: string): string | null {
  ...
  // Split by comma and get the first font (split always returns ≥1 element)
...
export async function waitForFont(
  ...
  try {
    ...
    await Promise.race([
      ...
      // ADAPT: template literal avoids no-restricted-syntax (NewExpression > Literal)
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`Font load timeout: ${family}`)), timeout),
      ),
    ])
