// RESEARCH: tweakcn (Apache-2.0) — copied from utils/theme-style-generator.ts
// See NOTICE.md.
//
// ADAPT: import paths → @/lib/design/* (our structure).
// ThemeEditorState inline-typed (no Zustand dep).
// colorFormatter: our version (no tailwindVersion param — Tailwind v4 only).
// getShadowMap → generateShadowLevels (our adapter signature).
// defaultLightThemeStyles → DEFAULT_LIGHT_COLORS from theme-defaults.
// extractFontFamily → @/lib/design/fonts/index.

import { AppError } from '@/lib/contracts/errors'
import { colorFormatter } from '@/lib/design/color-format'
import type { ColorFormat } from '@/lib/design/contract/index'
import type { ThemeStyleProps } from '@/lib/design/contract/theme'
import { extractFontFamily } from '@/lib/design/fonts/index'
import { generateShadowLevels } from '@/lib/design/shadows'
import { DEFAULT_LIGHT_COLORS } from '@/lib/design/theme-defaults'

type ThemeMode = 'light' | 'dark'
type ThemeStyles = { light: ThemeStyleProps; dark: ThemeStyleProps }
type ThemeEditorState = { currentMode: ThemeMode; styles: ThemeStyles }
...
const generateThemeVariables = (
  ...
  const shadowVars = generateShadowVariables(
    generateShadowLevels(themeStyles[mode]['shadow-color'], themeStyles[mode]),
  )
  const rawShadowVars = generateRawShadowVariables(themeStyles, mode)
  const spacingVar =
    mode === 'light'
      ? `\n  --spacing: ${themeStyles['light'].spacing ?? DEFAULT_LIGHT_COLORS.spacing};`
      : ''

  const trackingVars =
    mode === 'light'
      ? `\n  --tracking-normal: ${themeStyles['light']['letter-spacing'] ?? DEFAULT_LIGHT_COLORS['letter-spacing']};`
      : ''
...
export const generateThemeCode = (
  ...
  if (
    ...
    throw AppError.invalidInput({
      key: 'theme.invalid_styles',
      fallback: 'Invalid theme styles: missing light or dark mode',
    })
  }
...
type GoogleFontSlot = { importName: string; varName: string; cssVar: string }

const LAYOUT_COMMENT = `// For adding custom fonts with other frameworks, see:\n// https://tailwindcss.com/docs/font-family`

const buildLayoutNoFonts = (): string => `${LAYOUT_COMMENT}
...
const buildLayoutWithFonts = (googleFonts: GoogleFontSlot[]): string => {
...
export const generateLayoutCode = (themeEditorState: ThemeEditorState): string => {
  ...
  const lightStyles = (themeEditorState.styles as ThemeStyles).light
  const googleFonts: GoogleFontSlot[] = []

  for (const slot of FONT_SLOTS) {
    const fontFamily = extractFontFamily(lightStyles[slot.key])
    if (fontFamily) {
      googleFonts.push({
  ...
  return googleFonts.length === 0 ? buildLayoutNoFonts() : buildLayoutWithFonts(googleFonts)
}
