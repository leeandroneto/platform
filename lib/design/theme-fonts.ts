// RESEARCH: tweakcn (Apache-2.0) — copied from utils/theme-fonts.ts
// See NOTICE.md.
// ADAPT: import paths → @/lib/design/fonts/index + @/lib/design/contract/theme
// `getAppliedThemeFont` signature: accepts { styles, currentMode } shape
// (our Theme type + currentMode string) instead of ThemeEditorState (Zustand).

import type { Theme } from '@/lib/design/contract/theme'
import { SYSTEM_FONTS } from '@/lib/design/fonts/index'
...
/**
 * Get the applied font family name from a theme state.
 * ADAPT: accepts { styles: Theme, currentMode } instead of ThemeEditorState (Zustand).
 */
export const getAppliedThemeFont = (
  state: { styles: Theme; currentMode: 'light' | 'dark' },
