...
# docs/ é excluído por design: blueprints/ADRs/research legitimamente mencionam
# termos banidos em tabelas "use X em vez de Y" + matrizes de migração legacy.
# Vocab audit é pra código fonte (app/components/lib/supabase/messages/scripts).
HITS=$(
  grep -RInE "$BANNED" \
    --include='*.ts' --include='*.tsx' --include='*.md' --include='*.sql' --include='*.json' \
    app/ components/ lib/ supabase/ messages/ scripts/ 2>/dev/null \
