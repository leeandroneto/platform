# Migration 0001 — Schema baseline `public.*` + RLS + JWT hook + Templates
...
>
> **HISTÓRICO:** este documento descreve o estado FINAL após migrations
> 0001-0006. Originalmente migration 0001 criava `public.*` separado; foi
> consolidado em `public.*` via migration 0005 (ADR-0033, 2026-05-18).
> Para greenfield novo, recomenda-se aplicar tudo direto em `public.*`.
...
### Schema `public.*` brand + tenant (ADR-0024/0026/0028/0029)

| Tabela                                | Propósito                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `public.palettes`                   | Pool paletas (13 seed) + clones tenant — `(slug unique nullable, brand_id null, source_palette_id null FK self, created_by_tenant_id null FK tenants, name, OKLCH cols + surfaces_dark[5] + surfaces_light[5] + hue, is_official, is_active, sort_order)` (ADR-0028 + 0029 clone pattern)                                                                                                                                       |
| `public.fonts`                      | Pool 7 fontes — `(slug unique, brand_id null, display_name, family_name, provider, weights[], is_official, is_active)`                                                                                                                                                                                                                                                                                                          |
| `public.shape_presets`              | Pool 3 shapes — `(slug unique, brand_id null, display_name, radius_base/sm/md/lg/xl, is_official, is_active)`                                                                                                                                                                                                                                                                                                                   |
| `public.brands`                     | Brand filha — `(name, host, default_palette_id NOT NULL FK public.palettes, logo_url, default_vertical, parent_label, theme_version)`                                                                                                                                                                                                                                                                                         |
| `public.tenants`                    | Tenant root — `(brand_id FK, slug, vertical, palette_id NOT NULL FK public.palettes [DEFAULT subquery 'default'], font_id NOT NULL FK public.fonts [DEFAULT subquery 'geist'], shape_preset_id NOT NULL FK public.shape_presets [DEFAULT subquery 'rounded'], logo_url, theme_version, default_locale, default_currency, default_tz, pixels jsonb)`. **`custom_primary_oklch` REMOVIDO** (ADR-0029 clone pattern resolve) |
| `public.domains`                    | 1:N tenant — `(host, kind enum('subdomain'/'custom'), is_primary, verified_at, ssl_status)` (ADR-0026)                                                                                                                                                                                                                                                                                                                          |
| `public.profiles`                   | 1:1 com `auth.users`                                                                                                                                                                                                                                                                                                                                                                                                            |
| `public.memberships`                | `(tenant_id, user_id, role enum 5 valores: platform_admin/professional/client/influencer/service_account)` UNIQUE                                                                                                                                                                                                                                                                                                               |
| `public.subscriptions`              | Mensalidade plataforma EFI Bank — `(package enum A/B/C, money pair setup + monthly, grace_period_until)`                                                                                                                                                                                                                                                                                                                        |
...
| Tabela                         | Propósito                                                                                                                                                                                                                                |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `public.programs`            | Programa root — `(brand_id, tenant_id, vertical_id, source_template_id FK public.program_templates, source_template_version, title, slug, cover_image_url, money pair price, cohort_type enum, enrollment_window jsonb, status, tags[])` |
| `public.modules`             | `(program_id, title, position)`                                                                                                                                                                                                          |
| `public.components`          | `(tenant_id, module_id, kind enum 11 kinds, schema_version, payload jsonb CHECK ? 'schema_version', status)`                                                                                                                             |
...
| `public.capture_forms`       | Instância — `(tenant_id, source_template_id FK public.form_templates, fields jsonb, redirect_url)`                                                                                                                                       |
...
| `public.assessments`         | Relatório IA — `(tenant_id, lead_id, payload jsonb, model, created_at)`                                                                                                                                                                  |
| `public.push_subscriptions`  | Web Push subs — 1 par Vapid **por tenant** (RFC 8292) em `tenants.vapid_*`                                                                                                                                                               |
| `public.pages`               | **Instância** landing pública — `(tenant_id, source_template_id FK public.page_templates, source_template_version, slug, blocks jsonb CHECK ? 'schema_version', published_at, status)`                                                   |
| `public.page_versions`       | Snapshot por publish — `(page_id, version int, blocks_snapshot jsonb, published_at, published_by_user_id)`                                                                                                                               |

### Templates editáveis pelo prof (Pacote B/C)

| Tabela                     | Propósito                                                                                                   |
| -------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `public.push_templates`  | `(tenant_id null=global, template_key, lang, title, body, frequency_cap_minutes, enabled)`. Seed: 5 globais |
| `public.email_templates` | `(tenant_id null=global, template_key, lang, subject, body_html, body_text, enabled)`. Seed: 4 globais      |

**Total dia 0:** 33 tabelas em `public.*` + 12 tabelas em `public.*`
...
UNIQUE (tenant_id, slug) em public.{pages, programs, capture_forms}
UNIQUE (slug) em public.{page_templates, form_templates, program_templates, ai_prompts}
UNIQUE (slug) em public.{fonts, shape_presets}
UNIQUE (slug) WHERE is_official=true em public.palettes (clones não têm slug unique)
```
...
(0 tabelas em public.*). Se NÃO estiver vazio, ABORTA — projeto não é
greenfield e precisa criar um novo (ADR-0023). NÃO aplicar em project com legado.

Nome: 0001_initial

Conteúdo: schema public.* + RLS + JWT hook + triggers + 5 buckets +
...
7. CREATE public.palettes / fonts / shape_presets com audit cols + CHECK constraints
...
9. CREATE public.brands (default_palette_id NOT NULL FK public.palettes)
10. SEED brand 'desafit' com default_palette_id via subquery scalar
11. CREATE public.tenants SEM defaults nas FKs (apenas NOT NULL)
12. ALTER TABLE public.tenants:
    - ALTER COLUMN palette_id SET DEFAULT (SELECT id FROM public.palettes WHERE slug='default' AND is_official=true LIMIT 1)
    - ALTER COLUMN font_id SET DEFAULT (SELECT id FROM public.fonts WHERE slug='geist' LIMIT 1)
    - ALTER COLUMN shape_preset_id SET DEFAULT (SELECT id FROM public.shape_presets WHERE slug='rounded' LIMIT 1)
13. CREATE TRIGGER public.handle_new_user (cria profile+tenant+membership atomicamente)
14. CREATE public.{domains, profiles, memberships, subscriptions, tenant_gateway_credentials}
...
16. CREATE public.{programs, modules, components, component_schedules, enrollments, payments, capture_forms, capture_submissions, leads, assessments, push_subscriptions, pages, page_versions, push_templates, email_templates}
17. SEED templates globais:
    - 5 public.push_templates (tenant_id=null, template_keys: morning_workout, message_received, inactivity_3d, streak_milestone, weekly_checkin)
    - 4 public.email_templates (tenant_id=null, template_keys: enrollment_welcome, checkin_reminder, program_completed, dunning_attempt_1)
18. Aplicar audit cols + CHECK constraints + INDEXES baseline em TODAS tabelas
19. CREATE trigger public.on_tenant_soft_delete em public.tenants
...
- SET ROLE anon; INSERT em public.programs sem JWT → bloqueado ✅
...
- `public.tenant_branding` **não existe** — branding inline em `public.tenants` (ADR-0028 + 0029)
- `public.brands.primary_color_oklch` **não existe** — sempre via `default_palette_id` FK (ADR-0028)
- `public.tenants.custom_primary_oklch` **não existe** — paleta clonada via `public.palettes.source_palette_id` (ADR-0029)
