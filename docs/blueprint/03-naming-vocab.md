...
### URLs EN puras — multi-vertical compatible (ADR-0031 sucessor)

Rotas neutras a vertical (fitness, yoga, inglês, mentoria, etc) ficam em **EN
puro** — sem rewrite PT. Razão: `aluno`/`treinos` viola a marca filha "yoga.app"
(usuária = `praticante`) e "ingles.app" (usuária = `estudante`). Termos EN
genéricos (`portal`, `program`, `assessment`) servem todas as verticais.

| Rota                | Onde                                                            |
| ------------------- | --------------------------------------------------------------- |
| `/portal`           | `app/(client)/portal/` — área do cliente final (multi-vertical) |
| `/program/:slug`    | rota EN comum (futuro Sprint 3+)                                |
| `/assessment/:slug` | gerador IA — neutro a domínio                                   |

Rewrites PT-BR ficam reservados pra rotas **fitness-vertical only** (`/treinos`,
`/alunos`, `/programa/musculacao`). Quando uma rota for vertical-neutral, EN puro.
